...
llueve=function(){
  ...
  if (ruido==undefined){
    creaRuido()
    lloviendo=true;
    document.getElementById('botRain').style.backgroundColor="rgba(10,10,100,.5)";
    ...
    if(lloviendo){
      ...
      document.getElementById('botRain').style.backgroundColor="rgba(10,10,100,.7)";
      document.getElementById('botRain').style.color = "white";
...
volRu.oninput = function() {
    ruido.output.gain.value=this.value/100;
...
fluye=function(){/// para un setInterval
  console.log("fluyo",filtro.res())
  filtro.res(filtro.res()+(Math.random()-.5)/fluon);
}